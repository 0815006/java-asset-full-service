...
*   **定时任务与数据聚合**：引入了 `SchedulerConfig`，实现了每日凌晨 3 点自动清理 15 天前的过期数据（访问日志、阅读状态），以及每小时自动聚合“全行使用榜”和“资产人气榜”。
*   **Redis 缓存集成**：集成了 Redis 用于存储聚合后的榜单数据，显著提升了高频访问接口的响应速度。
*   **多源登录支持**：`LoginController` 现在支持从 `asset_user` 和 `sys_user` 两个数据源进行身份验证，并支持 AES 和 MD5 多种密码校验模式。
*   **密码修改功能**：新增了 `/api/user/change-password` 接口，允许用户在验证旧密码后修改新密码。
*   **Spring Security 深度集成**：实现了 `UserDetailsServiceImpl`，将系统用户体系与 Spring Security 标准认证流程进一步对齐。

### 改进

*   **系统用户模型**：新增了 `SysUser` 实体及对应的 Mapper，用于支持与外部系统用户数据的同步与集成。
*   **日志记录增强**：在登录流程中增加了详细的日志记录，便于追踪登录失败原因及用户来源。
